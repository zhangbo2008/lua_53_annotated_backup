...
/* execute a jump instruction */ //运行jump opt码.   i是opt, e是偏移量,对于test指令就写1,不是test的就写0 // 算法就是先拿到ra寄存器的偏移量.  ci->u.l.base + a - 1 是因为upvalue是在
...
/* for test instructions, execute the jump instruction that follows it */// 这个歌宏是用被test指令调用的.1是偏移量. 正常的jump是  dojump(ci, i, 0); 而test指令后面必须加的是一个jump指令,所以加1之后pc才到jump指令,然后最后一个变量,要写1.才对. i对应的指令还是test里面的. 
...
/* fetch an instruction and prepare its execution */ //有hook就调用traceexec,获取i里面的内容, 他的寄存器地址RA给ra //注意每一次上来就读取savedpc里面指令然后加加. 所以你如果其他指令里面savedpc++表示跳过指令, 因为vmfetch里面会自动++. 这才是遍历的++. 其他位置的++都是跳过的意思.
#define vmfetch()	{ \
  i = *(ci->u.l.savedpc++); \
 ...
 newframe:  /* reentry point when frame changes (call/return) */
  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_TEST) {//看op_test运行如何处理,看看<=>这个符号到底什么意思.
        if (GETARG_C(i) ? l_isfalse(ra) : !l_isfalse(ra)) // 如果有c参数,那么ra取反, 如果没有c,那么返回ra, 也就是说c如果是一个布尔值. c如果为true,那么ra true就不做,ra false就pc++, 如果c为false, 那么ra true就做,ra false就不做. 也就是ca同号,不做. ca不同号,就做.
            ci->u.l.savedpc++;// 这行表示成功,所以进入代码块.
          else
          donextjump(ci);//这行表示逻辑失败进行跳转. 因为test本质是if{代码块}end{代码块}, 这行逻辑是跳转所以表示test失败就跳转,否则++
        vmbreak;
      }
      vmcase(OP_TESTSET) {//再看这个.
        TValue *rb = RB(i);
        if (GETARG_C(i) ? l_isfalse(rb) : !l_isfalse(rb))//如果cb符号不相同,就进入代码块逻辑.
          ci->u.l.savedpc++;
        else {
          setobjs2s(L, ra, rb);//如果符号相同,那么就进行赋值.!!!!
          donextjump(ci);
        }
        vmbreak;
      }

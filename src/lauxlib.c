...
/*栈顶作为key往后找    val是否等于L[objidx]   //这个有点复杂. 举哥例子. 比如字典是 0 1 2 我们要找的是2那么设置level=3.之后开始从0遍历.然后一直拼接. 最后返回0.1.2即可.
...
static int findfield (lua_State *L, int objidx, int level) {
  if (level == 0 || !lua_istable(L, -1))
    return 0;  /* not found */
  lua_pushnil(L);  /* start 'next' loop */ //所以每次都从0开始遍历字典.
  while (lua_next(L, -2)) {  /* for each pair in table */  //lua_next返回值表示下一个是否存在.
    if (lua_type(L, -2) == LUA_TSTRING) {  /* ignore non-string keys *///-2位置存的是key
      if (lua_rawequal(L, objidx, -1)) {  /* found object? */
        lua_pop(L, 1);  /* remove value (but keep name) */
        return 1;
      }
      else if (findfield(L, objidx, level - 1)) { /* try recursively */  //等于每个元素找了level遍?????????因为每一次调用函数findfield时候49行都压入了一个nil,所以每次运行都不同.会往栈后面找.进行搜索.
        lua_remove(L, -2);  /* remove table (but keep name) */
        lua_pushliteral(L, ".");
        lua_insert(L, -2);  /* place '.' between the two names */
        lua_concat(L, 3);  //拼接这2个字符.
        return 1;
      }
    }
...
LUALIB_API int luaL_newmetatable (lua_State *L, const char *tname) {
  ...
  lua_setfield(L, -2, "__name");  /* metatable.__name = tname */ // 把栈顶放到L[-2][__name]
  lua_pushvalue(L, -1);
...
LUALIB_API lua_Integer luaL_checkinteger (lua_State *L, int arg) {
  int isnum;
  lua_Integer d = lua_tointegerx(L, arg, &isnum);// d=L[arg]
  if (!isnum) {
    interror(L, arg);
  }
  return d;
...
//读取数据存到lf->buff中.然后返回这个lf->buff, size表示读了多少.
...
//获取L[obj]的元表里面字段event对应的值. 如果陈宫就压入栈. 否则就无效果.
LUALIB_API int luaL_getmetafield (lua_State *L, int obj, const char *event) {
  if (!lua_getmetatable(L, obj))  /* no metatable? */
    ...
    tt = lua_rawget(L, -2); //L[-2][event] 注意tt放入栈顶,不是压栈.栈里面top不变.
...
/* upvalue是在函数之前压入的.  这里面就知道了upvalue是比函数提前入栈的.
